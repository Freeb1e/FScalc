...
void RamModel::eval_read_256(bool clk, uint64_t addr, RData256 &rdata) {
    bool is_posedge = (!last_clk && clk);

    if (is_posedge) {
        // 计算基地址
        // 因为 256bit = 4 * 64bit，所以外部地址 1 对应内部索引 4
        size_t base_index = addr * 4;

        // 检查边界：我们需要读取 base, base+1, base+2, base+3
        // 只要 base_index < memory.size()，我们就可以尝试读取
        // 越界部分填 0
        
        if (base_index < memory.size()) output_reg_256.d0 = memory[base_index];
        else output_reg_256.d0 = 0;

        if (base_index + 1 < memory.size()) output_reg_256.d1 = memory[base_index + 1];
        else output_reg_256.d1 = 0;

        if (base_index + 2 < memory.size()) output_reg_256.d2 = memory[base_index + 2];
        else output_reg_256.d2 = 0;

        if (base_index + 3 < memory.size()) output_reg_256.d3 = memory[base_index + 3];
        else output_reg_256.d3 = 0;
    }

    // 更新时钟记录（注意：如果是混合调用，这里可能会被其他 eval 覆盖，
    // 但在同一个仿真周期内通常只调一种 eval，或者 last_clk 逻辑是全局一致的）
    last_clk = clk;

    // 输出当前寄存器值
    rdata = output_reg_256;
}
// ============================================================
//  测试主函数更新 (增加了双端口测试)
// ============================================================
